...
1. 用 4 个 XOR 门将 `X2[3:0]` 与 `SUB = OP[0]` 按位异或，得到加法器第二输入 `B[3:0]`。
2. 用一个 4-bit 加法器计算 `SUM = X1 + B + Cin`，其中 `Cin = SUB`。
3. 并行计算两组逻辑结果：`X1 AND X2` 和 `X1 XOR X2`。
4. 用一个 4选1 MUX 在 `SUM`、`SUM`、`AND`、`XOR` 四路结果中选出最终输出 `Y[3:0]`。

> **注：** I0 和 I1 都可以连接同一个 `SUM` 输出。当 `OP=00` 时，`SUB=0`，所以 `SUM = X1 + X2`；当 `OP=01` 时，`SUB=1`，所以 `SUM = X1 - X2`。

---

## 电路原理图

下图为整理后的 4-bit ALU 电路原理图：
